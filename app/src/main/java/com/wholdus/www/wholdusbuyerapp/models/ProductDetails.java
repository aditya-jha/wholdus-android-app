...
public class ProductDetails {
    ...
    public void setDataFromCursor(Cursor cursor){
        mProductDetailsID = cursor.getInt(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_PRODUCT_DETAILS_ID));
        mUnit = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_UNIT));
        mDisplayName = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_DISPLAY_NAME));
        mDeleteStatus = (cursor.getInt(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_DELETE_STATUS))!=0);
        mShowOnline = (cursor.getInt(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_SHOW_ONLINE))!=0);
        mWarranty = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_WARRANTY));
        mSpecialFeature = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_SPECIAL_FEATURE));
        mAvailability = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_AVAILABILITY));
        mStyle = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_STYLE));
        mManufacturedCity = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_MANUFACTURED_CITY));
        mPattern = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_PATTERN));
        mLotDescription = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_LOT_DESCRIPTION));
        mDescription = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_DESCRIPTION));
        mWorkDescriptionType = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_WORK_DESCRIPTION_TYPE));
        mNeckCollarType = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_NECK_COLLAR_TYPE));
        mDispatchedIn = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_DISPATCHED_IN));
        mRemarks = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_REMARKS));
        mSellerCatalogNumber = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_SELLER_CATALOG_NUMBER));
        mSleeve = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_SLEEVE));
        mGender = cursor.getString(cursor.getColumnIndexOrThrow(CatalogContract.ProductsTable.COLUMN_GENDER));
