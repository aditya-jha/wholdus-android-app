package com.wholdus.www.wholdusbuyerapp.helperClasses;

import static java.util.logging.Logger.global;

/**
 * Created by aditya on 16/12/16.
 */

public final class Constants {

    static final String API_BASE = "http://api.wholdus.com/";
    static final String TEMP_API_BASE = "http://13.76.211.119:8000/";

    public static final String THUMB_IMAGE = "100x100";
    public static final String EXTRA_SMALL_IMAGE = "200x200";
    public static final String SMALL_IMAGE = "300x300";
    public static final String MEDIUM_IMAGE = "400x400";
    public static final String LARGE_IMAGE = "700x700";

    public static final String PRODUCT_RESPONSE = "ProductResponse";
    public static final String INSERTED_UPDATED = "insertedUpdated";

    public static final String ACTIVE_POSITION = "ActivePosition";
}
